...
const originAppName = {
  ...
  'http://consumertwo.ankuranand.in:3030': 'sso_consumer'
};
...
const verifySsoToken = async (req, res, next) => {
  console.log('i am in verification token');
...
const login = (req, res, next) => {
  ...
  console.log('My Session', req.session.user);
  if (req.session.user != undefined && serviceURL != null) {
    console.log('i am setting the ssotoken');
    const url = new URL(serviceURL);
    const intrmid = encodedId();
